...
<a href="' . $this->addToUrl($hrfNew) . '" class="' . $clsNew . '" title="' . StringUtil::specialchars($ttlNew) . '" accesskey="n" data-action="contao--scroll-offset#store">' . $lblNew . '</a>' : '') . ($security->isGranted(ContaoCorePermissions::DC_PREFIX . $this->strTable, new CreateAction($this->strTable, array('type' => 'file'))) ? '
			...
			if ($this->isMounted($currentFolder))
			{
				...
				else
				{
					...
					if (!$user->isAdmin && \in_array($currentFolder, $user->filemounts))
					{
						...
						if (!$security->isGranted(ContaoCorePermissions::DC_PREFIX.'tl_files', $subject))
						{
							$return .= Image::getHtml('new--disabled.svg') . ' ';
						}
						else
						{
